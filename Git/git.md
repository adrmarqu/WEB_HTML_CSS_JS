...
- [GIT i GITHUB - TUTORIAL COMPLET PER A MAC I WINDOWS](#git-i-github---tutorial-complet-per-a-mac-i-windows)
...
Instal·lació a Windows (usant VSCode)

- Descarrega l'instal·lador de Git per a Windows des de: https://git-scm.com/download/win i executa'l.
- Durant la instal·lació, accepta els paràmetres per defecte o escull opcions recomanades (afegir Git al PATH, gestionar saltos de línia amb `core.autocrlf`, etc.).
- Un cop instal·lat, obre **Visual Studio Code** i obre el terminal integrat (`View → Terminal` o `Ctrl+` `).`.
- Verifica la instal·lació amb:

    git --version

- Si el terminal de VSCode no troba `git`, indica el camí de l'executable a la configuració de VSCode. Obre la paleta de comandes (`Cmd/Ctrl+Shift+P`) i escull `Preferences: Open Settings (JSON)` i afegeix (exemple):

    "git.path": "C:\\Program Files\\Git\\bin\\git.exe"

- Opcional: instal·la extensions útils com `GitLens` per millorar l'experiència Git dins de VSCode.

Instal·lació a macOS (usant VSCode)

- Opció 1 (recomanada si tens Homebrew): obre el terminal integrat de VSCode i executa:

    brew install git

- Opció 2: instal·la les Xcode Command Line Tools executant en el terminal:

    xcode-select --install

- Verifica la instal·lació des de VSCode amb:

    git --version

- Si cal apuntar VSCode a un Git concret (per exemple Homebrew en Mac M1/M2), obre `Preferences: Open Settings (JSON)` i afegeix l'exemple:

    "git.path": "/opt/homebrew/bin/git"

Interfícies gràfiques i eines integrades

- [_SourceTree_](https://www.sourcetreeapp.com/)
- [_GitHub Desktop_](https://desktop.github.com/)
- [_GitKraken_](https://www.gitkraken.com/)
- [_Visual Studio Code_](https://code.visualstudio.com/) (té integració Git incorporada i moltes extensions com `GitLens`).
...
### Configuració des de Visual Studio Code

1. Obre **Visual Studio Code** i navega a la carpeta del teu projecte (`File → Open Folder`).
2. Obre el terminal integrat (`View → Terminal` o `Ctrl+` `).` i comprova que `git` està disponible:

    git --version

3. Executa les ordres de configuració globals des del terminal integrat per establir nom i correu, i altres opcions (exemple):
...
4. Ajustar la ruta de `git` a VSCode (si cal): obre la paleta (`Cmd/Ctrl+Shift+P`) → `Preferences: Open Settings (JSON)` i afegeix per exemple:
...
5. Configuracions des de la interfície de VSCode:

- Pots canviar opcions relacionades amb Git des de `Settings` (`Cmd/Ctrl+,`) buscant `Git`. Per exemple, habilitar `Git: Auto Fetch` o ajustar `Git: Confirm Sync`.
- Instal·la extensions com `GitLens` per visualitzar historial, blame, i millorar la integració amb VSCode.

6. Comprova la configuració actual des del terminal integrat:

    git config --list

Amb aquests passos tindràs Git instal·lat i configurat utilitzant l'entorn i les eines de Visual Studio Code.
...
### Per reemplaçar la branca _master_ per _main_ a _GitHub_
    ...
    #Pas 4
    #Canvia la branca_default_ de _màster_ a _main_ en el teu repositori de _GitHub_ .
