//Una función callback es aquella que es pasada como argumento a otra función para que sea "llamada de nuevo" (call back) en un momento posterior. Una función que acepta otras funciones como argumentos y no estás obligado a usar la palabra "callback" como el nombre de tu argumento, Javascript solo necesita saber que es el nombre correcto del argumento. Pueden ser una arrow function

//Cuando se usan funciones Callback
//La mayoría del tiempo estamos creando aplicaciones que operan en una forma sincrónica. En otras palabras, algunas de nuestra operaciones comienzan solo después de que se hayan completado las anteriores. Usualmente, cuando solicitamos datos desde otras fuentes como una API externa, no siempre sabemos cuando nuestros datos serán devueltos. En estos casos queremos esperar la respuesta, pero no queremos que toda nuestra aplicación se detenga mientras se recuperan los datos. Estas son situaciones dónde las funciones callback resultan útiles ya que aceleran los procesos que no dependen de la API externa.
...
const nuevoPais = (pais, callback) => {
 ...
 // los dos segundos ya han pasado, y se agrega uno nuevo, ejecutamos el callback para que se vuelva a llamar a la presentación en consola
};
//función callback
const mostrarPaises = () => {
  setTimeout(() => {
